const PassportOCRApp = {
    ...
    console.warn("OCR failed, proceeding with empty mock data");
}
    ...
    // Navigate immediately to verify page
    window.location.href = "verify.html";

} catch (err) {
    console.error(err);
    // Use fallback dummy data if backend fails
    const fallback = {
    firstName: "JOHN",
    lastName: "DOE",
    passportNumber: "A1234567",
    ...
    sessionStorage.setItem("ocrData", JSON.stringify(fallback));
    sessionStorage.setItem("passportImage", PassportOCRApp.state.capturedData);
    sessionStorage.setItem("phone", PassportOCRApp.state.phone);
    sessionStorage.setItem("email", PassportOCRApp.state.email);
    window.location.href = "verify.html";
